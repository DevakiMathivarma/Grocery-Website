...
cart.forEach((item, index) => {
    ...
    const row = document.createElement('tr');
    row.innerHTML = `
    <td><button class="x-btn" onclick="removeRow(this)" data-id="${item.name}" ><i class="fas fa-xmark"></i></button></td>
<td><img src="${item.image}" style="width:70px;",display:"inline-block;"><br>${item.name}<br><small >${item.weight}</small></td>
<td>₹${price.toFixed(2)}</td>
...
`;
    ...
    const html = `
<div class="order-details-text">
<h3 style="margin-bottom: 20px;">Thank You.Your Order has been Completed !</h3>
</div>
<div class="order-details-completed">
...
//  console.log(`Item ID: ${itemId.name}`);
